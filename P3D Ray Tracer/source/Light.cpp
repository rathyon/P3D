...
bool Light::inShadow(HitInfo& info, Grid& grid) {
	vec3 origin = info.intersection;
	vec3 L = normalize(_pos - origin);
	Ray feeler = Ray(origin + OFFSET * L, L);
	...
	HitInfo feelerInfo = grid.traverse(feeler);

	if (feelerInfo.t < light_t && feelerInfo.t != MISS) return true;
	else return false;
...
vec3 Light::shade(HitInfo& info, Grid& grid) {

	if (inShadow(info, grid))
		return vec3(0.0f);
